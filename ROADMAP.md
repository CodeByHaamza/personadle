...
- [ ] **Purger l'historique git — `.git` pèse 3,9 Go** _(mesuré le 2026-09-25, à faire hors
      semaine de release)_

  L'arbre de travail est passé de 2,5 Go à **1,2 Go** avec le réencodage des animations
  All-Out Attack (lot `perf/aoa_reencode_hd`). `.git`, lui, n'a pas bougé : les anciennes
  versions y restent. Un `git clone` télécharge donc toujours 3,9 Go.

  | Ce qui pèse                   | Poids     | État dans l'arbre          |
  | ----------------------------- | --------- | -------------------------- |
  | Anciens `.gif` All-Out Attack | ~1 335 Mo | supprimés depuis longtemps |
  | Anciens `.webp` 1080p         | ~1 500 Mo | remplacés le 2026-09-25    |
  | `.gif` d'avatars              | 14 Mo     | **vivants**, à garder      |

  Cible réaliste : **~600 Mo**.

  **Pourquoi ce n'est pas fait tout de suite**, et ce qu'il faut savoir avant de s'y mettre :

  - purger demande de **réécrire l'historique** (`git filter-repo`), donc un `push --force`
    sur `main` et `develop` ;
  - tous les SHA changent après le premier commit touché. **Le webroot Hostinger est un
    checkout de `main`** : son `git pull` automatique ne survit pas à une réécriture, il
    faudra le **recloner à la main** depuis hPanel → Avancé → GIT ;
  - toute copie locale du dépôt doit être reclonée, pas mise à jour ;
  - à faire **en une seule passe** (`.gif` + `.webp` ensemble), pas en deux : chaque
    réécriture coûte le même reclonage.

  ⚠️ **La contrainte qui décide de la méthode** : PersonaDLE doit rester **100 % jouable en
  local**. Git LFS est donc exclu — un clone sans `git lfs pull` ne donne que des fichiers
  pointeurs, donc un jeu cassé. Sortir les assets du dépôt pour ne les garder que sur R2
  l'est tout autant, et pour la même raison.
...
> **1525 tests JS · 379 PHPUnit · 278 E2E · PHPStan niveau 5 · CI/CD GitHub Actions**.

### Backend & Infrastructure

| #   | Fonctionnalité                                 | Notes                                                                             |
| --- | ---------------------------------------------- | --------------------------------------------------------------------------------- |
| B1  | Schéma BDD (30 tables, MySQL/MariaDB)          | `sql/bdd_mysql.sql` = source de vérité + migrations `sql/migrations/` (000→020)   |
...
| #   | Élément                                       | Notes                                             |
| --- | --------------------------------------------- | ------------------------------------------------- |
| Q1  | Tests : 1525 Vitest · 379 PHPUnit · 278 E2E   | `npm test` · `make test-php` · `npm run test:e2e` |
| Q2  | i18n EN/FR/ES/DE/IT/PT (1314 clés)            | `npm run i18n:check`                              |
