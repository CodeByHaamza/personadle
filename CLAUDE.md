...
├── lang/                ← en.json (source de vérité, 1297 clés), fr/es/de/it/pt.json
...
├── tests/               ← 77 suites Vitest (1448 tests) + tests/php/ (PHPUnit)
└── sql/                 ← bdd_mysql.sql (31 tables)
```
...
| Le client décide de ce qui existe | Un titre/badge n'est accordé que si **le serveur** le décide, pas si le client pense à le demander. Vécu le 2026-09-19 : `perfect_wins`, `titles_count`, `social_link_min_rank`, `expert_wins_total` n'avaient pas d'évaluation client → jamais demandés → **~290 titres et ~100 badges dus dormaient en prod**. Depuis : `api/lib/unlock_reconcile.php` accorde ce qui est dû à chaque `GET /api/titles` / `GET /api/badges`, et `BadgeWallpaperCatalogTest` exige que chaque entrée du catalogue soit accordée par cette réconciliation au seuil exact. Un nouveau `condition_type` = un `case` serveur + ce test, jamais une logique client seule |
...
- **1448 tests** (Vitest + jsdom), 77 suites dans `tests/` (`gameCore`, `backend`, `auth`, `i18n`,
