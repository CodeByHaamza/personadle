...
/**
 * Ce portrait bouge-t-il ?
 *
 * Le recadrage passe par un canvas, qui n'en garde qu'une image fixe : un
 * portrait animé qui y entre en ressort mort. La question n'est donc pas
 * cosmétique, c'est elle qui décide si on propose le recadrage.
 *
 * ── Pourquoi plus « le nom finit par .gif » ─────────────────────────────────
 * Cette règle était fausse des deux côtés — le commentaire de la pastille ANIM
 * le dit déjà quelques lignes plus haut, mais le garde-fou du recadrage, lui,
 * était resté dessus. Conséquence : les portraits animés de Kotone, qui sont des
 * **WebP** animés, passaient par le canvas et perdaient leur animation, tandis
 * que ceux de Makoto (`Yuki.gif`) l'échappaient et bougeaient très bien.
 * Signalé en production le 2026-09-25 : « les pdp de Kotone ont du mal à
 * s'animer, comme Makoto qui marche très bien ».
 *
 * `ANIMATED_AVATARS` est généré en relisant l'en-tête des fichiers : il dit ce
 * qui bouge réellement, pas ce que l'extension laisse croire.
 *
 * @param {string} src chemin ou nom de fichier
 */
export function isAnimatedAvatar(src) {
  const nom = String(src ?? "")
    .split("/")
    .pop();
  return ANIMATED_AVATARS.has(nom);
}
