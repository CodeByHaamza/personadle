...
| Commande | Vérifie |
|---|---|
| `make test` / `npm test` | 1448 tests JS (Vitest) |
| `make test-php` | 397 méthodes de test PHPUnit dans 23 fichiers (logique + intégration BDD). Tourne **dans le conteneur Docker** (`make up` requis) — pas besoin de PHP installé sur ta machine |
| `npm run lint` | ESLint |
