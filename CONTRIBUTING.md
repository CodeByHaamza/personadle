...
| Commande | Vérifie |
|---|---|
| `make test` / `npm test` | 1433 tests JS (Vitest) |
| `make test-php` | 368 méthodes de test PHPUnit dans 22 fichiers (logique + intégration BDD). Tourne **dans le conteneur Docker** (`make up` requis) — pas besoin de PHP installé sur ta machine |
| `npm run lint` | ESLint |
